...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('hr-up').addEventListener('click', () => { hours = Math.min(23, hours + 1); render(); });
  document.getElementById('hr-down').addEventListener('click', () => { hours = Math.max(0, hours - 1); render(); });
  document.getElementById('min-up').addEventListener('click', () => { mins = Math.min(55, mins + 5); render(); });
  document.getElementById('min-down').addEventListener('click', () => { mins = Math.max(0, mins - 5); render(); });
...
function render() {
  ...
  document.getElementById('min-val').textContent = String(mins).padStart(2, '0');
}
...
async function loadActivities() {
  try {
    ...
    if (!groupsRes.ok) { showLoginView(); return; }
...
async function handleLog() {
  ...
  if (!activityId) { msgEl.textContent = 'Select an activity.'; msgEl.className = 'message error'; return; }
  if (totalMins <= 0) { msgEl.textContent = 'Set a duration.'; msgEl.className = 'message error'; return; }
  ...
  try {
    ...
    if (res.ok) {
      const actName = activities.find(a => a.id === activityId)?.name || 'Activity';
      msgEl.textContent = `✓ ${totalMins}m of ${actName} logged!`;
      msgEl.className = 'message success';

      hours = 0; mins = 0; render();
