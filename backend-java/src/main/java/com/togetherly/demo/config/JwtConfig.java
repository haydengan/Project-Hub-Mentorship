package com.togetherly.demo.config;

import java.security.interfaces.RSAPrivateKey;
import java.security.interfaces.RSAPublicKey;
import lombok.Getter;
import lombok.Setter;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.context.annotation.Configuration;

/**
 * JWT token configuration — keys, lifetimes, issuer.
 *
 * The RSA key pair is loaded from application.properties via custom converters
 * (PrivateKeyConverter, PublicKeyConverter). Spring calls those converters
 * automatically when it sees @Value injecting a String into an RSAPrivateKey field.
 */
@Getter
@Setter
@Configuration
public class JwtConfig {
    @Value("${jwt.secret.privateKey}")
    private RSAPrivateKey privateKey;

    @Value("${jwt.secret.publicKey}")
    private RSAPublicKey publicKey;

    @Value("${jwt.access.token.lifetime:600}")
    private int accessTokenLifetimeSec;

    @Value("${jwt.refresh.token.lifetime:900}")
    private int refreshTokenLifetimeSec;

    @Value("${jwt.issuer:issuer}")
    private String issuer;

    @Value("${jwt.cookie.domain:.${jwt.issuer}}")
    private String cookieDomain;
}
